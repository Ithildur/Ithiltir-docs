...
| Dash 更新器 | Linux/systemd 发布包安装下调用包内更新脚本；任务状态和日志写入 `$DASH_HOME/runtime/dash-update` |
...
- Dash 自更新只支持单实例 Linux/systemd 发布包安装。任务由当前 Dash 进程启动，状态和最近日志写入 `$DASH_HOME/runtime/dash-update`，不写入 PostgreSQL。
