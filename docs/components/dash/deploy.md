...
Dash 格式 v1 发布包携带 Ithiltir-node 二进制和安装脚本，并通过 `/deploy` 分发给节点。

安装脚本模板公开可读。打包携带的节点二进制和 runner 资产需要 `X-Node-Secret`，或使用 Dash 为旧 Node 自动升级生成的短期 `upgrade_token`。安装脚本下载节点二进制时会发送 `X-Node-Secret`。

受保护资产响应使用 `Cache-Control: private, no-store`，并返回 `Vary: X-Node-Secret`，不得由共享缓存保存。

`release.env` 记录 Dash 版本、Node 版本、目标 OS/架构和下表 7 个资产的 SHA-256。更新器在停止在线服务前校验 manifest、全部摘要以及候选 Dash 报告的 Dash/Node 版本。
...
Linux、macOS 和 Windows 安装器手工处理重定向，最多 5 次。目标必须保持初始主机；同协议跳转必须保持有效端口；只允许 HTTP 升级到 HTTPS，不允许降级。通过校验后才向下一跳发送 `X-Node-Secret`。

Linux systemd 安装在存在 `cc`、`gcc` 或 `clang` 时编译 root 侧连接数 helper，并每秒刷新。OpenRC 和无法编译 helper 的主机使用 Node 内置统计，可能缺失容器连接数据。
...
省略 `--node-version` 时，脚本会从 `https://github.com/Ithildur/Ithiltir-node.git` 解析最新兼容 tag。正式打包还会检查 `dist/index.html` 和 `dist/theme-bootstrap.js` 非空，并且只把 `configs/config.example.yaml` 放入发布包。
