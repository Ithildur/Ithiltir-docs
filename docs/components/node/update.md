...
Dash 管理台的自动下发更新只适用于当前 Node 版本为 `0.2.3` 或更高的节点。低于 `0.2.3` 的节点必须重新执行安装命令或手工替换二进制。
...
与当前上报版本相同的 manifest 会先被忽略。如果同一轮里剩余多个 target 返回 update manifest，所有 manifest 的 `id`、`version`、`url`、`sha256` 和 `size` 必须一致；有冲突则跳过更新。
